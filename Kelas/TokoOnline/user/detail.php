...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $size = $_POST['sizes']; // Get selected size from form input
    ...
    // Check if product with selected size already exists in cart
...
// Fetch sizes from database but filter only M, L, XL, XXL, 3XL
$sizes_to_fetch = ['M', 'L', 'XL', 'XXL', '3XL'];
$placeholders = implode(',', array_fill(0, count($sizes_to_fetch), '?'));
$query_sizes = "SELECT size_name FROM sizes WHERE size_name IN ($placeholders)";
$stmt_sizes = $conn->prepare($query_sizes);

// Bind parameters dynamically
$stmt_sizes->bind_param(str_repeat('s', count($sizes_to_fetch)), ...$sizes_to_fetch);
$stmt_sizes->execute();
                    ...
                    <div class="sizes">
                        <?php foreach ($sizes as $size): ?>
                            <label class="size-item">
                                <input type="radio" name="sizes" value="<?= htmlspecialchars($size) ?>" required>
